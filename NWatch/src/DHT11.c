 ...
 * @Date: 2023-12-27 16:12:33
 * @LastEditors: lunuj 68381451+lunuj@users.noreply.github.com
 * @LastEditTime: 2024-02-17 17:58:27
 * @FilePath: /NWatch/src/DHT11.c
 * @Description:
 *
...
void DHT_Test()
{
    uint8_t buffer[5];
    while (1) {
        int a = DHT_Get_Temp_Humi_Data(buffer);
        OLED_ShowNum(4, 1, a, 2);
        if (a != 0) {
            OLED_ShowNum(2, 1, (buffer[0] << 8) + buffer[1], 6);
            OLED_ShowNum(3, 1, (buffer[2] << 8) + buffer[3], 6);
        }
    }
}