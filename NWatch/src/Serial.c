...
u16 Serial2_RxFlag = 0;

u8 Serial2_TxCach[4];
u8 Serial2_RxCach[400];
...
void Serial2_SendNum(int Num)
{
    int thumb = Num / 10;
    if (thumb != 0) {
        Serial2_SendNum(thumb);
    }
    Serial2_SendByte(Num % 10 + '0');
}
...
void Serial2_SendPack(void)
{
    ...
    Serial2_SendArrary(Serial2_TxCach, 4);
...
{
    // static u8 RxState = 0;
    static u8 Serial2_pRxPack = 0;
    if (USART_GetFlagStatus(USART2, USART_IT_RXNE) == SET) {
        u8 RxData = USART_ReceiveData(USART2);
        Serial2_RxCach[Serial2_pRxPack++] = RxData;
        Serial2_pRxPack           = 0;
        Serial2_RxFlag            = 1;
    }
}
